...
class DataStructMon : KoinComponent {
    ...
    companion object {
        const val LOG_TLG_MAX = 16
        const val LOG_EVENT_MAX = 100
    }
    ...
    fun dispStatus(): MonitorStatus {
       ...
       */
        ...
        for (i in 0..3) {
             ...
             */
            dmonitor_rel = String.format(getString(R.string.relay_num), i + 1)
            dmonitor_wiper = dispR_Wiper[i]
            dmonitor_learn = dispR_Learn[i]
            dmonitor_programs = dispR_ProgEn[i]
            dmonitor_loop = dispR_LoopEn[i]
            dmonitor_transmiterfail = dispR_TransmitFail[i]
            dmonitor_switchingcnt = dispR_BrPrek[i]
            monitorStatus.relayStatuses.add(
                RelayStatus(
                    dmonitor_rel,
                    dmonitor_wiper,
                    dmonitor_learn,
                    dmonitor_programs,
                    dmonitor_loop,
                    dmonitor_transmiterfail,
                    dmonitor_switchingcnt
                )
            )
        }
        status += dmonitor_rel
